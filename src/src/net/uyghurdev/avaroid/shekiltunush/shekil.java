...
public class shekil extends Activity {
	public int  Width;
	public  int  Height;
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    this.getWindowManager().getDefaultDisplay().getMetrics(dm);
	       ...
	       if(Width==320&&Height==480){
	    	   setContentView(R.layout.shekil320_480);
	    	   
	       }
	       else if(Width==480&&Height==800){
	    	   setContentView(R.layout.shekil480_800);
	    	   
	       }
	       else if(Width==768&&Height==1024){
	    	   setContentView(R.layout.shekil768_1024);
	...
	public void Next() {
		...
		imvw1.setBackgroundResource(n == 0 ? lzm.myimg[30] : lzm.myimg[n - 1]);
		rlt1.startAnimation(right_out);
		txt1.setText(n == 0 ? lzm.shekilname[30] : lzm.shekilname[n - 1]);
		
		imvw2.setBackgroundResource(lzm.myimg[n]);
		rlt2.startAnimation(right_in);
		txt2.setText(lzm.shekilname[n]);
	...
	public void back() {
		...
		imvw1.setBackgroundResource(n == 30 ? lzm.myimg[0] : lzm.myimg[n + 1]);
		rlt1.startAnimation(left_out);
		txt1.setText(n == 30 ? lzm.shekilname[0] : lzm.shekilname[n + 1]);
		imvw2.setBackgroundResource(lzm.myimg[n]);
...
public void anim_out(){
	...
	public void player(int i) {
		...
		if (sound == true) {
			...
			mMediaPlayer = MediaPlayer.create(shekil.this, lzm.mymp3[i]);
			mMediaPlayer.start();
		} else if (sound == false) {
			if (mMediaPlayer != null) {
	...
	private void About(Context context) {
		...
		builder.setTitle("شەكىل تونۇش");
		builder.setMessage("               نەشرى1.1.0 ");
